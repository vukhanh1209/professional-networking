...
export default function CoverLetterSection({defaultCoverLetter} :any) {
    ...
    return (
        <section className="bg-white rounded-lg px-6 pt-6 pb-8 text-primary-black">
            <div className={`${defaultCoverLetter && "border-b-2 border-silver-grey"} flex items-center justify-between w-full pb-4`}>
                <h3 className="text-xl mb-3 font-bold">
                    Thư xin việc
                </h3>
                <button onClick={onShowEditingSection} className={`${isEditing && "hidden"}`}>
                    <Image src={Edit} width={20} height={20} alt="edit"/>
                </button>
            </div>

            {isEditing ?
                <form onSubmit={handleSubmit(onSaveChange)} className="flex flex-col  mt-6">
                    ...
                    <span className="text-sm text-[#979595]">{`${maxLength - value.length}/${maxLength} ký tự`}</span>
