...
import { recruiterForgotPassword } from "@/redux/actions/recruiter.action";
  ...
  const onSubmit = async (data: any) => {
    if (data) {
      ...
      if (pathName.includes("for-recruiter")) {
        const res = await dispatch(recruiterForgotPassword(resBody));
        handleServiceResponse(res);
        if (res.meta.requestStatus === "fulfilled") {
          setTimeout(() => {
            router.push(`/for-recruiter/account/reset-password/${data.email}`);
          }, 500);
        }
      } else {
